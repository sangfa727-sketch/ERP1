...
export default function SuppliersPage() {
  ...
  const fetchAll = async () => {
    ...
    const { data } = await supabase.from('contacts')
      .select('*').eq('contact_type','Supplier').eq('is_deleted',false).order('contact_name')
    setContacts(data || [])
    setLoading(false)
  }
  useEffect(() => { fetchAll() }, [])
    ...
    <AppLayout>
      <div className="p-4 md:p-6 max-w-5xl mx-auto">
        <div className="flex items-center justify-between mb-4">
          <h1 className="text-xl md:text-2xl font-bold text-gray-800">{`🏪 ${t.page_suppliers}`}</h1>
          <button onClick={() => setModal({...EMPTY})} className="px-4 py-2 bg-blue-600 text-white rounded-lg text-sm font-medium">{t.btn_add}</button>
        </div>
        <div className="mb-4">
        ...
        <div className="md:hidden space-y-3">
          ...
          : filtered.map(c => (
            <div key={c.id} className={`bg-white rounded-xl shadow-sm p-4 border-l-4 ${Number(c.current_balance) > 0 ? 'border-yellow-400' : 'border-green-400'}`}>
              <div className="flex justify-between items-start mb-2">
                ...
                <div className="text-right">
                  <span className={`font-bold text-sm ${Number(c.current_balance) > 0 ? 'text-red-600' : 'text-gray-600'}`}>
              ...
              <div className="flex gap-2 mt-2">
                ...
                <button onClick={() => handleDelete(c.id)} className="flex-1 py-1.5 bg-red-500 text-white rounded-lg text-xs">{t.btn_delete}</button>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="hidden md:block bg-white rounded-xl shadow-sm overflow-hidden">
          <table className="w-full text-sm">
            ...
            <tbody>
              ...
              : filtered.length === 0 ? (<tr><td colSpan={5} className="text-center p-8 text-gray-400">{t.no_data}</td></tr>)
              : filtered.map(c => (
                <tr key={c.id} className="border-b hover:bg-gray-50">
                  <td className="p-3 font-medium">{c.contact_name}</td>
      ...
      {modal && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-end md:items-center justify-center z-50">
          <div className="bg-white rounded-t-2xl md:rounded-xl p-5 w-full md:max-w-md shadow-xl max-h-[90vh] overflow-y-auto">
            <h2 className="text-lg font-bold mb-4">{modal.id ? '✏️ ' + t.btn_edit : '+ ' + t.page_suppliers}</h2>
