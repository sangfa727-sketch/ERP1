...
export default function SignupPage() {
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    if (password.length < 6) { setError('Password အနည်းဆုံး ၆ လုံး ထည့်ပါ'); return }
    if (!companyName.trim()) { setError('Company နာမည် ထည့်ပါ'); return }
    setLoading(true)
    ...
    if (!userId) { setError('User တည်ဆောက်မအောင်မြင်ပါ'); setLoading(false); return }

    // Create company + profile via secure function
    const { error: fnError } = await supabase.rpc('fn_signup_create_company', {
      p_auth_user_id: userId,
      p_company_name: companyName.trim(),
      p_full_name: companyName.trim() + ' Admin',
    })
      ...
      <div style={{
        ...
        {success ? (
          <div style={{
            ...
            <p style={{color:'#64748b', fontSize:'0.8rem', marginTop:'4px'}}>Email verify လုပ်ပြီးမှ login ဝင်နိုင်ပါသည်</p>
            <p style={{color:'#94a3b8', fontSize:'0.75rem', marginTop:'8px'}}>Login page သို့ ပြန်သွားနေသည်...</p>
          </div>
        ) : (
          <>
            {error && (
            ...
            <div style={{marginTop:'1.5rem', paddingTop:'1rem', borderTop:'1px solid #e2e8f0', textAlign:'center'}}>
              ...
              <a href="/login" style={{fontSize:'0.8rem', color:'#2563eb', fontWeight:'600', textDecoration:'none'}}>
                Login ဝင်ပါ →
              </a>
            </div>
          </>
